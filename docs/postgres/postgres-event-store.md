...
- `sequence_position` is a `BIGSERIAL` primary key providing a monotonically increasing global order. Note that `BIGSERIAL` allocates values at INSERT time but rows only become visible at COMMIT, so concurrent writers can commit out of allocation order. See [The Read Barrier](design.md#the-read-barrier) for how this is masked from readers and subscribers.
- `type` uses `COLLATE "C"` for byte-level equality -- faster than locale-aware collation and sufficient for event type strings.
- `tags` is a `TEXT[]` array with a GIN index (built `fastupdate=off`). Tag filtering uses array containment (`@>`) against that index within the type-filtered subset.
